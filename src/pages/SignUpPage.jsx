...
export default function SignUpPageV2() {
  ...
  const { signup, loading, error: authError } = useSessionAuth()
  ...
  async function handleSubmit(e) {
    ...
    setIsLoading(true)
    const { success, error: signupErr } = await signup(email, password, rememberMe)

    if (success) {
      navigate(redirectTo)
    } else {
      setError(signupErr || 'Signup failed')
      setIsLoading(false)
    }
  }
      ...
      <div style={{ width: '100%', maxWidth: 400 }}>
        <div style={{ background: C.bg2, borderRadius: 16, border: `1px solid ${C.border}`, padding: '36px 32px', boxShadow: '0 4px 24px rgba(15,31,61,0.06)' }}>
          ...
          <p style={{ ...F.sans, fontSize: 14, color: C.muted, margin: 0, marginBottom: 28 }}>
            Get started with BindIQ for free
          </p>
