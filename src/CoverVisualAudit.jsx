...
export default function CoverVisualAudit({file,trimWidth,trimHeight,spine}){
    ...
    async function run(){
      ...
      try{
        ...
        const safeBoxes={
          ...
          spine:{x:spineLeft+SAFE/2,y:BLEED+SAFE,w:Math.max(0,spine-SAFE),h:trimHeight-SAFE*2},
          barcode:{x:BLEED+trimWidth-BARCODE.edge-BARCODE.width,y:BLEED+BARCODE.edge,w:BARCODE.width,h:BARCODE.height},
        }
        const found=[]
        for(const item of text.items){
          ...
          const w=(item.width||0)/INCH
          const h=Math.max(fontHeight,.01)
          const box={x,y,w,h,text:(item.str||'').trim()}
        ...
        setIssues(unique)
        setSummary({textItems:text.items.length,pageWidth,pageHeight,spineLeft,spineRight})
        setStatus('')
  ...
  function drawGuides(ctx,scale,{trimWidth,trimHeight,spine,pageHeight}){
    ...
    ctx.setLineDash([8,6]);ctx.strokeStyle='rgba(220,38,38,.9)'
    ctx.strokeRect(px(BLEED),px(BLEED),px(trimWidth*2+spine),px(trimHeight))
    ctx.strokeStyle='rgba(109,40,217,.95)'
    ctx.beginPath();ctx.moveTo(px(trimWidth+BLEED),0);ctx.lineTo(px(trimWidth+BLEED),h);ctx.moveTo(px(trimWidth+BLEED+spine),0);ctx.lineTo(px(trimWidth+BLEED+spine),h);ctx.stroke()
    ctx.strokeStyle='rgba(22,163,74,.95)'
    ctx.strokeRect(px(BLEED+SAFE),px(BLEED+SAFE),px(trimWidth-SAFE*2),px(trimHeight-SAFE*2))
    ctx.strokeRect(px(BLEED+trimWidth+spine+SAFE),px(BLEED+SAFE),px(trimWidth-SAFE*2),px(trimHeight-SAFE*2))
    ctx.strokeStyle='rgba(234,88,12,.95)'
    ctx.strokeRect(px(BLEED+trimWidth-BARCODE.edge-BARCODE.width),px(pageHeight-(BLEED+BARCODE.edge+BARCODE.height)),px(BARCODE.width),px(BARCODE.height))
    ctx.restore()
  }
