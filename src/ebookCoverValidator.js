export function evaluateEbookCover({width,height,type}){
  const ratio=width/height
  const issues=[]
  if(!['image/jpeg','image/png'].includes(type)) issues.push('Use a JPEG or PNG cover image.')
  ...
  return {width,height,ratio,type,issues,pass:issues.length===0}
}
...
export async function inspectEbookCover(file){
  ...
  try{
    ...
    return {...evaluateEbookCover({width:image.naturalWidth,height:image.naturalHeight,type:file.type}),size:file.size}
  } finally {
